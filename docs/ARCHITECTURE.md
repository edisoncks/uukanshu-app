...
child of its load, so a superseded revalidate can never commit stale totals. Scaffold chrome: TopAppBar (back + book line 1 + position/total/title line 2 via pure ReaderHeader; `windowInsets=0`, outer NavHost padding owns status height once (Detail + Reader both zero); static divider under bar mirroring bottom) + bottom [⋯ | 上一章 | 下一章]; ⋯ opens a ModalBottomSheet mirroring Settings rows (theme radios via selectable, language Switch, font stepper with bound-disable + preview). Nav bounds live in the VM (prev()/next()->AtEnd/Started, last-tapped wins); deleted pageId (-1) maps to ReaderErrorKind.Deleted with 該章節已刪除 + 回到目錄 instead of looping retry. Bodies split via pure ReaderParagraphs (inverse of ChapterParser \n\n join) in a SelectionContainer. Out-of-range and load errors show retry. |
