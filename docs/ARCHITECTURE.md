...
|---|---|
| Home (`ui/home`) | Tabs 最近更新 (recent) / 分類 (category). Category chips (`AssistChip`), Paging 3 `LazyColumn` (one HTML page per load; Paging 3 prefetches the next page automatically — no hand-rolled prefetcher). `BookPagingSource` filters already-seen ids per list because the live recent feed overlaps pages; one cached Pager per tab/category (`pagingFor`, keyed `recent`/`cat-{id}`) so ids never leak across lists and detail→back replays cached pages instead of refetching page 1 (a `_ui.flatMapLatest { Pager }` Flow would rebuild the Pager on every re-collection and lose scroll). Refresh/append load-states drive the spinner, error + retry and empty states. Scroll is per-list: `rememberSaveable(key="home-$key")` + ViewModel map restores on detail→back (HOME entry is retained); explicit tab/category switches (`selectTab`/`selectCategory`) set a one-shot `pendingTop` flag consumed via `scrollToItem(0)`. |
| Search (`ui/search`) | `queries` flow (versioned keystrokes + 400ms `debounce` + `flatMapLatest` into `repo.search()`: superseded searches cancel structurally with no manual guard, and an identical-text retry refires instead of conflating away), dedup by book id, follows `Prefs.simplified` live. Sealed `Ui` (Idle/Loading/Success/Error): impossible states are unrepresentable and the `when` is exhaustive. `Loading` carries the last results so a new query keeps stale rows under a thin bar (spinner only with no rows). Shows result count (共 … 條結果), loading / error + retry / 沒有結果 (no results) states. |
| Detail (`ui/detail`) | Sealed `Load` (Loading/Failed/Ready) split from live overlays (download progress, cache badges, bookmark, prefs): TOC stale-while-revalidate is one cold producer (`data/repo/TocSource` + `TocState`: `Ready(Syncing)` thin bar → one terminal emission) collected into `Load`; offline/refreshing are derived from the phase, never tracked. Offline-with-cache keeps content + 離線模式 flag, offline-without-cache shows error + retry (retry restarts the producer once per call). An empty or shrunken fresh TOC is treated as refresh failure (never wipes painted cache); the 追更 badge clears exactly on accepted `Fresh`. Live flows for badges (`cachedPositionsFlow: Set<pageId>`, never positions — positions shift on TOC inserts and would mislabel badges) and bookmark (`bookmarkFlow` pageId-based via `resolveBookmark`, rows keyed by `pageId`) compose orthogonally. Full-novel download is app-scoped (re-reads cached TOC at end and fetches growth delta so success never lies about 800/800 when 805 exist; Detail markSeen-after-download requires manager total to cover live TOC) (`BookDownloadManager` survives leaving detail, re-attaches on re-open; one book at a time — a second tapped book queues — sequential chapters, cancelable, progress `done/downloadTotal` from the manager, coerced 0..1); refresh never cancels a running download. Scaffold chrome: TopAppBar (back + book title, `windowInsets=0`, static divider; TopBar owns title). |
| Reader (`ui/reader`) | Loads by 1-based `position` + stable `pageId` into TOC (`resolveEffectivePosition` prefers `pageId`, pure + tested). Sealed `Ui` (Loading/Content/Error with position/total/prefs on the interface for the sticky bottom bar) — content+spinner combos are unrepresentable. Cache-first chapter text (read/written by stable `pageId`, so shifts can't misfile), silent auto-bookmark (`saveProgress(bookId, position, pageId)`, failures ignored), silent prefetch of next 5 (sequential with crawl delay, failures ignored). TOC is one `TocSource` producer per screen VM (`TocState`: cache paint `Syncing` → one terminal `Fresh`/`Stale`; empty/shrunken/failed refresh never wipes painted cache, and `total` never zeroes into bogus "out of range"). A chapter load owns one generation atomically (`snapshot`) and may trigger bounded restarts — one self-restart on no-cache `Failed`, plus at most one more when its target misses the current generation (max 2 fetches per `load()`, never a loop) — the old blocking-fetch rescue, bounded and gen-guarded (single collector + `tocGen` invalidation drops late emits; a late `Fresh` can only bump `total`/title, never position or content). Prev/next serialize via `loadJob` (last-tapped wins); a no-cache TOC failure self-restarts once per load, and the retry button restarts it again (bounded). Scaffold chrome: TopAppBar (back + book line 1 + position/total/title line 2 via pure ReaderHeader; `windowInsets=0`, outer NavHost padding owns status height once (Detail + Reader both zero); static divider under bar mirroring bottom) + bottom [⋯ | 上一章 | 下一章]; ⋯ opens a ModalBottomSheet sharing the Settings Prefs/VM source but reader-optimized widgets (same keys, same VM): segmented language buttons (both options visible), font slider + stepper with bound-disable + live preview, appearance as one row of tonal theme tiles. Nav bounds live in the VM (prev()/next()->AtEnd/Started, last-tapped wins); deleted pageId (-1) maps to ReaderErrorKind.Deleted with 該章節已刪除 + 回到目錄 instead of looping retry. Bodies split via pure ReaderParagraphs (inverse of ChapterParser \n\n join) in a SelectionContainer. Out-of-range and load errors show retry (retry re-resolves by Error pageId so a TOC shift cannot alias to a neighbor). |
| Library (`ui/library`) | Sealed `Load` (Loading/Failed/Shelf with optional footer error) split from live overlays (per-book download states, fresh-download titles, 追更 checking/lastCheck): stale-while-revalidate keeps rows on return-refresh, spinner only for the initial empty load, refresh failures surface as footer retry with rows or full-screen without. Lists `repo.libraryFlow()` (single owner; `repo.library()` one-shot kept for tests) (`CachedBook`: id/title/author/cached/total/bytes/newChapters), total `N 本 · size`, 追更 badge `有更新 N章` + tab dot + `檢查更新` (Main-guard, thin bar, footer retry; silent 6h foreground auto-check (the Worker's Doze fallback), off with the 追更 auto switch), `上次檢查` from `Prefs.lastBookCheck`, per-book 刪除緩存 (delete) + 清空全部 (clear all, behind a confirm dialog). Deletes also evict retained `BookDownloadManager` state so re-opened details can't replay stale progress (terminal publishes after `forget()` are dropped; old-job `finally` never evicts a new job). Failed downloads offer 重試 on the shelf row itself. Progress publishes are dropped once the job is gone, so cancel can't lose to an in-flight callback. Live `BookDownloadManager` progress per book (`下載中 done/total` + 取消, incl. fresh downloads not yet qualified for the shelf). While downloading, the shelf hides the stale `cached/total` count and shows only the live `done/total` line so progress never appears twice. Empty state hints at downloading from the detail page. |
| Settings (`ui/settings`) | Four cards: 外觀 (appearance radio: 自動/淺色/深色), 語言 (language switch 繁體/簡體), 追更 (auto switch 自動檢查更新 → WorkManager KEEP/UPDATE/CANCEL + POST_NOTIFICATIONS request, 上次檢查 text, denied-perm helper), 更新 (current version 目前版本, 檢查更新 button, checking spinner, up-to-date note, reopen-dismissed-update button, 跳過此版本 skip). Writes go to `Prefs` so all screens follow live. |
| Update (`ui/update`) | `UpdateDialog` renders every updater state (checking / up-to-date / error / new-version / downloading / file-ready / needs-unknown-sources) from `UpdateViewModel.Ui`; the transitions themselves are pure (`UpdateDecisions`). `startDownload` checks the on-disk APK first, so a complete file skips the unknown-sources gate straight to install. See "In-app update" below. |
...
- `data/repo/BookRepo.kt` (`SiteGateway` interface, not concrete `SiteApi`)
  ...
  the same failure shape: strict shrink guard (`TocSource` consumption check via
  `TocRevalidator.shouldAcceptFresh` + `TocShrunkException` from `BookRepo.detail`)
  fails closed before `replaceToc` so a truncated parse
...
- Reader and Detail prefer cache, then network, then save raw — all content
  ...
  misfile text (no caller-side shift guard needed). Next-5 prefetch lives
  in `ReaderViewModel`; both it and full download are sequential with
  `crawlDelay()` and never break reading on failure.
...
(`UpdateViewModel`, `UpdateDecisions`, `UpdateDialog`). Gateways make the VM
fakeable; every non-trivial dialog-state decision lives in pure
`ui/update/UpdateDecisions.kt` — `UpdatePolicy` (24h throttle + offer/skip),
the `canStartCheck`/`canStartInstall` gates behind the Main-thread
`getAndUpdate` test-and-sets, and the single terminal
`classifyDownloadSuccess`/`applyDownloadSuccess` verdict, which pins a receipt
to the enqueued release and skips hashing when the release ships no digest
(JVM-tested in `UpdateDecisionsTest`). `UpdateViewModel` is the effect shell; it takes an
injected `ioDispatcher` (default `Dispatchers.IO`) so its tests run on the test
scheduler instead of polling real threads (`UpdateViewModelTest`,
`UpdateIntegrityVmTest`, plus one real-IO `UpdateRealIoSmokeTest`).
