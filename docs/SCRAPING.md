...
- Timeouts: connect 30s, read 30s (interactive); bulk crawl work runs 15s/15s
  so a stuck background fetch cannot wedge the gate. Socket-level `callTimeout`
  per profile (interactive 90s, bulk 45s) aborts interrupt-ignoring reads at the
...
- Single-flight (`UukanshuGate`): at most one uukanshu.cc request
  in flight, always. Bulk chapters (full download, prefetch) run under
  `withContext(BulkFetch)` and use the bulk timeouts above. In-flight is never
  preempted (preempting would abort user downloads); a tap waits at most one
  bounded bulk attempt because backoff and `crawlDelay` run outside the gate.
  New bulk loops must opt into `BulkFetch` — the default is interactive timeouts.
